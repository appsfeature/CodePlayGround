...
class TimeOutHandler(application: Application) : ActivityLifecycleCallbacks {
    ...
    private var isValidDateTime = false
    ...
    fun resetScreenTimeOutTask(context: Context) {
        if (isValidDateTime && isValidScreenSaverList) {
            callback?.let {
            ...
            logSS("isValidDateTime: $isValidDateTime &&  isValidScreenSaverList:$isValidScreenSaverList")
            job?.cancel()
        }
    }

    private fun isValidDateTimeToShow(): Boolean {
        return (System.currentTimeMillis() > POSAdvertiseUtility.getTimeInMillis(POSScreenSaver.getStartDate())
                && System.currentTimeMillis() < POSAdvertiseUtility.getTimeInMillis(POSScreenSaver.getEndDate()))
    }
    ...
    private fun isValidAllConditions(): Boolean {
        isValidDateTime = isValidDateTimeToShow()
